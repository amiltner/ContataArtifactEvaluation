...
module type StateS = State.S

module ClassifiedType = struct
  module TermClassification = struct
    type t =
      | Introduction
      | Elimination
...
struct
  ...
  type t =
    | Vals of Id.t * (Value.t * Value.t) list * ClassifiedType.t
    | Top
  ...
  let imply
    ...
    begin match (s1,s2) with
      | (_,Top) -> true
      | (Top,_) -> false
      | (Vals (i1,vps1,(t1,_)),Vals (i2,vps2,(t2,_))) ->
        if Type.equal t1 t2 && Id.equal i1 i2 then
          List.for_all
            ~f:(fun (v2,p2) ->
  ...
  let normalize
    ...
    begin match x with
      | Top -> Top
      | Vals (i,vps,(t,_)) -> Vals (i,vps,(t,ClassifiedType.TermClassification.Introduction))
    end

  let top = Top

  let vals i vvs t = Vals (i,vvs,t)

  let print a b = pp b a

  let product a b =
    begin match (a,b) with
      | (Vals (i1,avs,at), Vals (i2,bvs,bt)) ->
        if ClassifiedType.equal at bt && Id.equal i1 i2 then
          Some (Vals (i1,(avs@bvs),at))
        else
          None
      | (Top, _) -> Some b
      | (_, Top) -> Some a
...
struct
  ...
  let get_angelic_call
      (v:t)
    : Id.t option =
    begin match fst3 (node v) with
      | AngelicCall i -> Some i
      | _ -> None
    end

  let is_angelic_call
    : t -> bool =
    Option.is_some % get_angelic_call
  ...
  let print_id id =
    ...
    | Var -> "Var"
    | LetIn -> "LetIn"
    | AngelicCall i -> "AngelicCall(" ^ Id.show i ^ ")"
...
module LogicToTSO = DictOf(UFForm)(OptionOf(A.TermState))
...
type t =
  {
    ...
    mutable min_term_state : LogicToTSO.t           ;
...
let rec term_to_exp_internals
  ...
  begin match Transition.id t with
    ...
    | AngelicCall i ->
      begin match ts with
        | [t] ->
          Expr.mk_app (Expr.mk_var i) (term_to_exp_internals t)
...
let term_to_contextful_exp
    (id:Id.t)
    (tin:Type.t)
    (tout:Type.t)
    (t:A.Term.t)
  : ContextLanguage.Expr.t =
  let internal = term_to_exp_internals t in
  let internal = ContextLanguage.Expr.from_standard_exp internal in
  (ContextLanguage.Expr.Func ((xid,tin),internal))
...
let rec term_to_safe_eval_internals
  ...
  begin match Transition.id t with
    ...
    | AngelicCall i ->
      begin match ts with
        | [t] ->
          SafeEval.App (SafeEval.Var i,SafeEval.Check (term_to_safe_eval_internals t))
        | _ -> failwith "incorrect"
      end
    | VariantSwitch is ->
      begin match ts with
  ...
  let rec term_to_angelic_exp
    ...
    AngelicEval.(begin match Transition.id t with
        ...
        | AngelicCall i ->
          begin match ts with
            | [t] ->
              AngelicF (i,(term_to_angelic_exp t))
            | _ -> failwith "incorrect"
          end
        | VariantSwitch is ->
          begin match ts with
  ...
  begin match Type.node desired_t with
    ...
    | Variant branches ->
      List.fold
        ...
        ~init:None
        branches
    | Universal _ -> failwith "try to not do"
  end
...
let extract_unbranched_states
  ...
  : State.t list =
  List.map ~f:(State.normalize % A.TermState.get_state) (extract_unbranched_states_internal ts)
(*begin match Transition.id t with
   | Apply ->
     begin match ts with
       | [t1;t2] ->
...
let invalidate_computations
    (c:t)
  : unit =
  c.min_term_state <- LogicToTSO.empty

let get_final_values
    (c:t)
    (vin:Value.t)
  : (Value.t * UFForm.t) list =
  let final_states_and_forms = A.get_finals_and_constraints c.a in
  let ungrouped_logics =
    (List.concat_map
       ~f:(fun (s,form) ->
           begin match s with
             | Vals (_,(vinsvouts),_) ->
       ...
       final_states_and_forms)
  in
  let grouped_logics = group_by ~key:fst ~equal:Value.equal ungrouped_logics in
  let integrated_logics =
    List.map
      ~f:(fun l ->
          fold_on_head_exn
            ~f:(fun (v1,form1) (_,form2) ->
                (v1,UFForm.bor form1 form2))
            l)
      grouped_logics
  in
  integrated_logics
...
let val_state
  ...
  State.vals c.id vinsvouts (t,cl)
...
let add_state
  ...
  begin
    ...
    && ClassifiedType.TermClassification.equal cl ClassifiedType.TermClassification.Introduction
...
let remove_transition
    ...
    (sout:State.t)
  : unit =
  A.remove_transition c.a trans constrt sins sout;
  invalidate_computations c
...
let evaluate
    (c:t)
    (input:Value.t list)
    (f:Value.t list -> (Value.t * UFForm.t) list)
    (args:State.t list)
    ((t,cl):ClassifiedType.t)
  : (State.t * UFForm.t) list =
  ...
  List.map
    ~f:(fun outs ->
        let (in_outs,constrts) = List.unzip (List.map2_exn ~f:(fun i (o,constrt) -> ((i,o),constrt)) input outs) in
        (val_state c in_outs (t,cl),UFForm.and_ constrts))
    outsl
(*let args =
  List.map
    ~f:(List.map
...
let update_from_conversions
    ...
    (conversions:
       ((Transition.id
         * (Value.t list -> (Value.t * UFForm.t) list)
          ...
          && List.length tins > 0 then
            ...
            List.concat_map
              ~f:(fun ins ->
                  ...
                  List.map
                    ~f:(fun (out,constrt) -> (i,ins,out,constrt,fst tout))
                    outs)
              args_list)
      conversions
  in
...
let initialize_context
  ...
  let c =
    {
      ...
      input_type        ;
      output_type       ;
      min_term_state = LogicToTSO.empty;
      all_states        ;
      value_filter      ;
      dupe_ref = None   ;
      id                ;
  ...
  in
  let duped_ins = List.map ~f:(fun i -> (i,i)) input_vals in
  let _ =
    add_transition
      c
      Var
      []
      (val_state c duped_ins (input_type,ClassifiedType.TermClassification.Elimination))
      UFForm.true_
      input_type
      true
  in
  let _ =
    add_transition
      ...
      Var
      []
      (val_state c duped_ins (input_type,ClassifiedType.TermClassification.Introduction))
      UFForm.true_
      input_type
      true
  in
    ...
    ~f:(fun i ->
        let _ =
          add_transition
            ...
            Var
            []
            (val_state c [(i,i)] (input_type,ClassifiedType.TermClassification.Elimination))
            UFForm.true_
            input_type
            true
        in
        let _ =
          add_transition
            ...
            Var
            []
            (val_state c [(i,i)] (input_type,ClassifiedType.TermClassification.Introduction))
            UFForm.true_
            input_type
            true
        in
...
let initialize_empty
  ...
  let c =
    initialize_context
      ...
      (fun _ _ -> true)
  in
  A.add_final_state c.a State.Top;
  c
...
let add_destructors
  ...
  let all_transformations =
    List.concat_map
      ~f:(fun (t:Type.t) ->
          let ct = (t,ClassifiedType.TermClassification.Elimination) in
...
let intersect
    ...
    (c2:t)
  : t =
  if List.is_empty c1.inputs then c2 else if List.is_empty c2.inputs then c1 else
  Consts.time
    Consts.isect_times
    (fun () ->
       let merge_tset
       ...
       let a = A.intersect (List.map ~f:(fun ts -> (ts,UFForm.true_)) ts_initial) c1.a c2.a in
...
let rec contains_angelic_call
    (Term (t,ts):A.Term.t)
  : bool =
  (Transition.is_angelic_call t)
  || (List.exists ~f:contains_angelic_call ts)
    ...
    let rec term_cost_internal
      ...
      if List.mem ~equal:A.Term.equal !terms fullt then
        ...
        begin match Transition.id t with
          ...
          | Var -> (*0.*)
            1.
          | AngelicCall _ -> (*1. +. simple_subcost ts*)
            1. +. simple_subcost ts
...
let rec extract_recursive_calls
    (ts:A.TermState.t)
  : (Id.t * State.t * State.t) list =
  begin match ts with
    | TermState (t,target,[source_ts]) ->
      begin match Transition.get_angelic_call t with
        | Some i -> (i,A.TermState.get_state source_ts,target)
                    ::(extract_recursive_calls source_ts)
        | None ->
          List.concat_map
...
let rec extract_recursive_calls_partial
    (ts:A.TermState.t)
  : (A.TermState.t * State.t) list =
  begin match ts with
    | TermState (t,target,[source_ts]) ->
      if Transition.is_angelic_call t then
        (source_ts,target)::(extract_recursive_calls_partial source_ts)
      else
        List.concat_map
          ~f:(extract_recursive_calls_partial)
          [source_ts]
    | TermState (_,_,tss) ->
      List.concat_map
        ~f:(extract_recursive_calls_partial)
...
let calls
  ...
  List.dedup_and_sort ~compare:(pair_compare Value.compare Value.compare)
    (List.concat_map
       ...
       (extract_recursive_calls_partial ts))
...
let is_valid_term
  ...
  begin match Transition.id t with
    | AngelicCall _ ->
      (List.exists ~f:contains_var ts) && not (List.exists ~f:contains_angelic_call ts)
    | VariantConstruct i ->
      begin match ts with
        | [Term (t,_)] ->
          begin match Transition.id t with
...
(* This was used for heuristic, lets see if necessary *)
let is_valid_ts
    (ts:A.TermState.t)
  : bool =
...
let min_term_state
  ...
  begin match LogicToTSO.lookup c.min_term_state form with
    | Some mts -> mts
    | None ->
      let mts =
        let basic_mts =
          Consts.time
            ...
            (fun _ -> A.min_term_state c.a ~f:is_valid_ts (*TODO, remove*) ~cost ~base_formula:form)
        in
      ...
      in
      let mts_dict = LogicToTSO.insert c.min_term_state form mts in
      c.min_term_state <- mts_dict;
      mts
  end
...
let rec_
    ...
    (i:Id.t)
  : Transition.t =
  Transition.create (Transition.AngelicCall i, c.output_type, 1)

let all_accepting_logic
    (c:t)
    (term:A.Term.t)
  : UFForm.t =
  A.all_accepting_logic c.a term


(*module ExtractorSingleton =
struct
  let rec extract_recursive_calls
      (ts:(Transition.t,State.t) TimbukSimple.TermState.t)
    : (Id.t * (Transition.t,State.t) TimbukSimple.TermState.t * State.t) list =
    begin match ts with
      | TermState (t,target,[source_ts]) ->
        begin match Transition.id t with
          | AngelicCall i ->
            (i,source_ts,target)::(extract_recursive_calls source_ts)
          | _ ->
            extract_recursive_calls source_ts
        end
      | TermState (_,_,tss) ->
  ...
  type t = (Transition.t,State.t) TimbukSimple.TermState.t -> UFForm.t

  let extract_recursive_requirements
      (i:Id.t)
      (sin:(Transition.t,State.t) TimbukSimple.TermState.t)
      (sout:State.t)
    : (Value.t * Id.t * Value.t * Value.t * (Transition.t TimbukSimple.Term.t)) list =
    begin match (State.destruct_vals (TimbukSimple.TermState.get_state sin),State.destruct_vals sout) with
      | (Some (vvsin,_), Some (vvsout,_)) ->
        let TermState (tn, _, _) = sin in
        let t = TimbukSimple.TermState.to_term sin in
        let outs = List.map ~f:snd vvsout in
        let inouts = List.zip_exn vvsin outs in
        List.map ~f:(fun ((exv,vsin),vsout) -> (exv,i,vsin,vsout,t)) inouts
      | (None, None) ->
        []
      | _ -> failwith "when would this happen?"
    end
  ...
  let value
    ...
    UFForm.and_
      (List.map
         ~f:(uncurry3 UFForm.fc)
         (List.dedup_and_sort ~compare:(triple_compare Id.compare Value.compare Value.compare)
            (List.concat_map
               ~f:(fun (i,ts,s) ->
                   let vss = extract_recursive_requirements i ts s in
                   List.map ~f:(fun (_,i,v1,v2,_) -> (i,v1,v2)) vss)
