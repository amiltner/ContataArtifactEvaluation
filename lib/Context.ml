...
module T = struct
  ...
  type 'a id_list = (Id.t * 'a) list
  [@@deriving eq, hash, ord, show]

  let pp caller fmat map = pp_id_list caller fmat (Sequence.to_list (Map.to_sequence map))
end
      ...
      | Variant branches ->
        Type.mk_variant
          (List.map
             ...
             branches)
      | Arrow _ | Mu _ | Universal _ -> t
    end
  in
  let t = Map.find_exn tc (Id.create "t") in
  let tv = Type.destruct_id_exn t in
  let t = Map.find_exn tc tv in
