...
let is_functionless
  ...
  fold
    ~name_f:(fun _ -> true)
    ~arr_f:(fun _ _ -> false)
    ~tuple_f:(fun ss -> List.for_all ~f:Fn.id ss)
    ~mu_f:(fun _ _ -> true)
    ~variant_f:(List.fold ~f:(fun acc (_,b) -> acc && b) ~init:true)
    ~universal_f:(fun _ -> true)
