...
module Expr = struct
  ...
  let fold (type a)
    ...
    let rec fold_internal (e:t) : a =
      ...
      | Abstract (id,i)
        -> abstract_f id i
      | LTE (e1,e2) -> lte_f (fold_internal e1) (fold_internal e2)
    in fold_internal e
  ...
  let apply_abstract
    ...
    : a option =
    begin match node e with
      | Abstract (id,i) -> Some (f id i)
      | _ -> None
    end

  let destruct_abstract
    : t -> (Id.t * int) option =
    apply_abstract ~f:(fun id i -> (id,i))

  let destruct_abstract_exn
      (e:t)
    : Id.t * int =
    Option.value_exn (destruct_abstract e)
  ...
  let rec replace
    ...
    begin match node e with
      ...
      | Abstract (id,i) ->
        e
      | LTE (e1,e2) ->
        mk_lte (replace_simple e1) (replace_simple e2)
    end
  ...
  let rec contains_var
    ...
    begin match node e with
      ...
      | Abstract _ -> false
      | LTE (e1,e2) -> contains_var_simple e1 || contains_var_simple e2
    end
  ...
  let rec simplify
    ...
    begin match node e with
      ...
      | Abstract _ -> e
      | LTE (e1,e2) -> mk_lte (simplify e1) (simplify e2)
    end
  ...
  let size : t -> int =
    fold ~var_f:(fun _ -> 1)
      ...
      ~proj_f:(fun _ i -> i+2)
      ~abstract_f:(fun _ i -> i)
      ~lte_f:(fun i1 i2 -> 1+i1+i2)
    ...
    and pp_internal
      ...
      if !Consts.pretty_ctors && is_nat_literal e then
        ...
        begin match node e with
          ...
          | Abstract (id,i) -> fpf ppf "%a#%d" Id.pp id i
          | LTE (e1,e2) ->
            fpf ppf "%a <= %a" pp_internal (this_lvl + 1,e1) pp_internal (this_lvl + 1,e2)
        end;
        (if this_lvl < lvl then fpf ppf ")")
  ...
  let rec extract_unbranched_switches
    ...
    begin match node e with
      ...
      | Abstract _ -> []
      | LTE (e1,e2) -> (extract_unbranched_switches e1)@(extract_unbranched_switches e2)
    end
...
module Value = struct
  ...
  let mk_abstract (id:Id.t) (i:int) : t =
    create (Abstract (id,i))

  let apply_abstract (type a) ~(f:Id.t -> int -> a) (v:t) : a option =
    begin match node v with
      | Abstract (id,i) -> Some (f id i)
      | _ -> None
    end

  let destruct_abstract : t -> (Id.t * int) option =
    apply_abstract ~f:(curry Fn.id)

  let destruct_abstract_exn (v:t) : Id.t * int =
    Option.value_exn (destruct_abstract v)
