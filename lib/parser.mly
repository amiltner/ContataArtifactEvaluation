...
unprocessed_problem:
    | ids=imports_decls SYNTH st=synth_targets SATISFYING ds=decl_list s=spec EOF
      { (fst ids,snd ids,st,ds,s) }

synth_targets:
    | t=target SEMI SEMI ts=synth_targets
      { t::ts }
    |
      { [] }

target:
      | l=LID COLON LBRACKET tin=typ RBRACKET ARR tout=typ CALLING LBRACKET ls=lids RBRACKET
      { (CoreAndMore.Id.create l,(tin,tout,ls)) }

typ_list:
    | ts=nonempty_typ_list
           { ts }
    | { [] }

nonempty_typ_list:
    | t=typ COMMA ts=nonempty_typ_list
      { t::ts }
    | t=typ
      { [t] }
...
nonempty_lids:
    | l=LID COMMA ls=nonempty_lids
      { CoreAndMore.Id.create l::ls }
    | l=LID
      { [CoreAndMore.Id.create l] }
...
spec:
    | f=single_spec SEMI SEMI
      {[f]}
    | f1=single_spec SEMI SEMI f2=spec
      {f1::f2}
...
single_spec:
    | TEST f=formula
      { Test f }
    | POST i=LID e=exp
      { Post (CoreAndMore.Id.create i,e) }
    | IO i=LID ex=example
      { IO (CoreAndMore.Id.create i,fst ex,snd ex) }
    | EQUIV i=LID e=exp
      { Equiv (CoreAndMore.Id.create i, e) }
...
typ:
  ...
  | t=typ_mu      { t }
  | t=typ_variant { t }
  | i=UID         { Type.mk_universal (CoreAndMore.Id.create i) }
...
typ_tuple_list:
  ...
  | i=UID STAR ts=typ_tuple_list_one { Type.mk_universal (CoreAndMore.Id.create i) :: List.rev ts }
...
typ_tuple_list_one: (* NOTE: reversed *)
  ...
  | ts=typ_tuple_list_one STAR i=UID { Type.mk_universal (CoreAndMore.Id.create i) :: ts }
...
exp_base:
  ...
  | e1=exp_base LTE e2=exp_base
    { Expr.mk_lte e1 e2 }
  | LPAREN PIPE id=UID num=INT RPAREN
    { Expr.mk_abstract (CoreAndMore.Id.create id) num }
  | LPAREN e=exp RPAREN
    { e }
  | LPAREN RPAREN
    { Expr.mk_unit }
