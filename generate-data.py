...
BASE_FLAGS = []
TIMEOUT_TIME = 120
CORRECT_TIMEOUT_TIME = 60
STILL_WORK_TIMEOUT_TIME = 60
...
def gather_datum(prog, path, base, additional_flags, timeout):
    start = time.time()
    flags = additional_flags + ["-log", "cegis"]
...
def gather_data(rootlength, prog, path, base,name,run_smyth):
    ...
    gather_col([],ctime_combiner,["IsectTotal","IsectMax","MinifyTotal","MinifyMax","MinEltTotal","MinEltMax","InitialCreationTotal","InitialCreationMax","AcceptsTermTotal","AcceptsTermMax","LoopCount","FullSynth","FullSynthMax","Z3","Z3Max","ComputationTime"],TIMEOUT_TIME,REPETITION_COUNT,False)
    gather_col(["-use-angelic"],ctime_combiner,["AngelicIsectTotal","AngelicIsectMax","AngelicMinifyTotal","AngelicMinifyMax","AngelicMinEltTotal","AngelicMinEltMax","AngelicInitialCreationTotal","AngelicInitialCreationMax","AngelicAcceptsTermTotal","AngelicAcceptsTermMax","AngelicLoopCount","AngelicFullSynth","AngelicFullSynthMax","AngelicZ3","AngelicZ3Max","AngelicComputationTime"],TIMEOUT_TIME,REPETITION_COUNT,False)
    gather_col(["-use-simple"],ctime_combiner,["SimpleIsectTotal","SimpleIsectMax","SimpleMinifyTotal","SimpleMinifyMax","SimpleMinEltTotal","SimpleMinEltMax","SimpleInitialCreationTotal","SimpleInitialCreationMax","SimpleAcceptsTermTotal","SimpleAcceptsTermMax","SimpleLoopCount","SimpleFullSynth","SimpleFullSynthMax","SimpleZ3","SimpleZ3Max","SimpleComputationTime"],TIMEOUT_TIME,REPETITION_COUNT,False)
    #if run_smyth:
...
def main(args):
    if len(args) == 3:
        ...
        rootlength = len(path)
        if os.path.exists(prog) and os.path.exists(path) and os.path.isdir(path):
