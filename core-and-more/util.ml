...
module EitherOf(L:Data)(R:Data) = struct
  type t = (L.t,R.t) either
  [@@deriving ord, show, hash, eq]
end
...
let fold_on_nonempty_head ~f:(f:'a -> 'a -> 'a) (x:'a) (l:'a list) : 'a =
  begin match l with
    | [] -> x
    | _ -> fold_on_head_exn ~f:f (x::l)
  end

let fold_on_head_with_default ~f:(f:'a -> 'a -> 'a) ~default:(d:unit -> 'a) (l:'a list) : 'a =
  begin match l with
    | [] -> d ()
...
let group_by_keys_from_basis
  ...
  List.fold_left
    ~f:(fun acc (k,v) ->
        insert_into_correct_list ~is_eq:is_eq acc k v)
    ~init:(List.map ~f:(fun x -> (x,[])) keys_list)
    kvl
