...
struct
  type t =
    | Vals of (Value.t * Value.t) list * Contata.FTAConstructor.ClassifiedType.t
    | Top
  ...
  let imply
    ...
    begin match (s1,s2) with
      ...
      | (Vals (vps1,(t1,_)),Vals (vps2,(t2,_))) ->
        if Contata.Type.equal t1 t2 then
          List.for_all
            ~f:(fun (v2,p2) ->
  ...
  let destruct_vals
      (x:t)
    : ((Value.t * Value.t) list * Contata.FTAConstructor.ClassifiedType.t) option =
    begin match x with
      | Vals (vs,t) -> Some (vs,t)
      | _ -> None
    end
  ...
  let destruct_vals_exn
      (x:t)
    : ((Value.t * Value.t) list * Contata.FTAConstructor.ClassifiedType.t) =
  ...
  let normalize
    ...
    begin match x with
      | Top -> Top
      | Vals (vps,(t,_)) -> Vals (vps,(t,Contata.FTAConstructor.ClassifiedType.TermClassification.Introduction))
    end
  ...
  let product a b =
    begin match (a,b) with
      | (Vals (avs,at), Vals (bvs,bt)) ->
        if Contata.FTAConstructor.ClassifiedType.equal at bt then
...
struct
  type id =
    ...
    | AbstractT of (Id.t * int)
  [@@deriving eq, hash, ord, show]
  type t_node = (id * Contata.Type.t * int)
  [@@deriving eq, hash, ord, show]
  ...
  let print_id id =
    ...
    | Apply ->
      "Apply"
    | AbstractT (id,i) ->
      "AbstractT (" ^ Id.show id ^ "," ^ Int.to_string i ^ ")"
...
module Reqs = struct
  ...
  let partial_compare
    ...
    let r1r2impl = Contata.UFForm.valid (Contata.UFForm.implies r1 r2) in
    let r2r1impl = Contata.UFForm.valid (Contata.UFForm.implies r2 r1) in
    begin match (r1r2impl,r2r1impl) with
      | (true,true) -> PO_EQ
      | (true,false) -> PO_GT
      | (false,true) -> PO_LT
  ...
  let and_ = Contata.UFForm.and_
  let or_ = Contata.UFForm.or_
  let false_ : t = Contata.UFForm.false_
  let true_ : t = Contata.UFForm.true_
end
...
module Make(A : Automaton.S with module Symbol := Transition and module State := State)(ReqCaller : Singleton with type t = A.TermState.t -> A.Reqs.t) = struct
  module TypeDS = struct
    include
      DisjointSetWithSetDataOf
        (struct
          include Contata.Type
          let preferred t1 t2 =
            begin match (Contata.Type.node t1,Contata.Type.node t2) with
              | (Variant _,Variant _)
              | (Arrow _,Arrow _)
              | (Tuple _,Tuple _) ->
                Contata.Type.size t1 < Contata.Type.size t2
              | _ ->
                begin match Contata.Type.node t1 with
                  | Variant _
                  | Arrow _
                  | Tuple _ -> true
                  | _ -> false
        ...
        (BoolModule)
        (struct
          type t = Contata.Type.t -> bool
          let v = (Option.is_some % Contata.Type.destruct_mu)
        end)
        (struct
  ...
  module TypeToStates = DictOf(Contata.FTAConstructor.ClassifiedType)(StateSet)
  ...
  let rec term_of_type ds t =
    ...
    begin match Type.node desired_t with
      ...
      | Universal id ->
        Some
          (TimbukSimple.Term.Term
             (Transition.create (AbstractT (id,0),desired_t,0)
             ,[]))
    end
  ...
  let get_states
      (c:t)
      ((t,cl):FTAConstructor.ClassifiedType.t)
    : State.t list =
  ...
  let add_state
    ...
    begin
      ...
      && FTAConstructor.ClassifiedType.TermClassification.equal cl FTAConstructor.ClassifiedType.TermClassification.Introduction
  ...
  let update_from_conversions
      ...
      (conversions:
         ((Transition.id
           ...
           * (FTAConstructor.ClassifiedType.t list) * FTAConstructor.ClassifiedType.t) list))
      ...
      ~f:(fun i ->
          let _ =
            add_transition
              ...
              Var
              []
              (val_state c [(i,i)] (input_type,FTAConstructor.ClassifiedType.TermClassification.Elimination))
              input_type
              true
          in
          let _ =
            add_transition
              ...
              Var
              []
              (val_state c [(i,i)] (input_type,FTAConstructor.ClassifiedType.TermClassification.Introduction))
              input_type
              true
          in
          ())
  ...
  let add_destructors
    ...
    let all_transformations =
      List.concat_map
        ~f:(fun (t:Type.t) ->
            let ct = (t,FTAConstructor.ClassifiedType.TermClassification.Elimination) in
  ...
  let add_states
      (c:t)
      (states:((Value.t * Value.t) list * FTAConstructor.ClassifiedType.t) list)
    : bool list =
    List.map
      ~f:(fun (vvs,t) ->
          add_state c vvs t)
    ...
    begin match ts with
      | TermState (t,target,[source_ts]) ->
        begin match Transition.get_angelic_call t with
          | Some i -> (i,TimbukSimple.TermState.get_state source_ts,target)
  ...
  let rec contains_destruct
      (Term (t,ts):A.Term.t)
    : bool =
    begin match Transition.id t with
      | VariantSwitch _ -> true
      | _ -> List.exists ~f:contains_destruct ts
    end

  let min_term_state
      (c:t)
      (viable_reqs:A.Reqs.t -> bool)
        ...
        let mts =
          let basic_mts =
            Consts.time
              ...
              (fun _ -> A.min_term_state c.a ~f:is_valid_ts ~cost ~reqs:ReqCaller.value ~viable_reqs)
          in
...
struct
  ...
  type t = (Transition.t,State.t) TimbukSimple.TermState.t -> Contata.UFForm.t
